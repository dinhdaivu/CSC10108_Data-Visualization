...
def get_quadrant_data(df, year):
    """Tính toán Điểm TB và Độ lệch chuẩn của 9 môn (Siêu nhẹ)"""
    df_y = df[df['nam'] == year][list(SUBJECT_NAMES.keys())]
...
def plot_quadrant_chart(df_stats):
    """Vẽ biểu đồ Góc phần tư: Phân hóa vs Độ khó"""
    if df_stats.empty: return go.Figure()

    # Tính điểm cắt (Trung vị) để chia 4 góc
    ...
    # Thêm trục tọa độ chéo (Crosshair) chia 4 góc phần tư
...
def render(df):
    ...
    # HÀNG 2: MA TRẬN PHÂN HÓA ĐỀ THI (Quadrant Chart)
