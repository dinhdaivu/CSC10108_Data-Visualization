\section{Data Collection, Construction, and Preprocessing}
    ...
    \textbf{Note}: The extraction procedures described in this section were programmatically implemented and executed via the \textbf{\textit{products.py}} script.
        ...
        \begin{itemize}
            \item \textbf{Objective}: To systematically collect all product metadata from the Nike Flagship Store on Lazada. The target data fields include product name, selling price, quantity sold, rating score, and inventory status.
            \item \textbf{Approach}: The system utilizes a Hybrid Web Scraping methodology. This approach combines browser automation (via Selenium) to bypass initial security mechanisms such as Captchas with direct HTTP requests to optimize extraction speed and efficiency by querying hidden internal APIs.
        \end{itemize}
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                \item The system initializes an automated browser instance (Google Chrome) using \textbf{Selenium} and \textbf{WebDriverManager}.
            ...
            \begin{itemize}
                ...
                \item \textbf{Data Cleaning}: Unstandardized data formats are processed immediately. For instance, text strings representing sales volume (e.g., "1.5k sold") are converted into standard integer formats (1500) using regular expressions to facilitate downstream quantitative analysis.
                \item \textbf{Anti-bot Evasion Mechanism}: The script implements randomized delays using \texttt{time.sleep}. Each pause lasts about 2 to 3 seconds between page requests. This simulates organic human browsing behavior and helps reduce the risk of IP blocking or rate-limiting by the platform's anti-scraping firewalls.
            ...
            \begin{itemize}
                \item The system dynamically generates a hierarchical directory structure based on the current execution date (\texttt{data/MM-DD-YYYY}).
                \item The data is simultaneously exported into two formats:
                \begin{itemize}
                    \item \textbf{CSV File} (\texttt{products.csv}): Intended for end-users to easily read, filter, and visualize the data using spreadsheet software like Microsoft Excel.
                    \item \textbf{JSON File} (\texttt{products.json}): Preserves the raw structured format, acting as the requisite input file for the subsequent phase of the project (Review Collection Process).
                \end{itemize}
            \end{itemize}
        \end{itemize}
    ...
    \textbf{Note}: The extraction procedures described in this section were programmatically implemented and executed via the \textbf{\textit{reviews.py}} script.
        ...
        \begin{itemize}
            ...
            \begin{itemize}
                \item To mitigate the risk of data loss, the extracted reviews are appended to the local storage immediately after processing each individual product. Furthermore, if the system encounters a rate-limit or an anti-bot block signal (e.g., returning a \texttt{FAIL\_SYS\_USER\_VALIDATE} error), it halts the loop, automatically reopens the Selenium browser to allow for manual Captcha resolution, regenerates the security tokens, and seamlessly resumes the crawling process from the exact point of interruption.
            \end{itemize}
        \end{itemize}

    \subsection{Dataset Construction}
    After the crawling stage, the raw product and review files were transformed into a relational analytical model for Power BI. This construction step is important because the later dashboard pages do not operate directly on the raw crawler outputs. Instead, the project reorganizes the collected data into dimension tables, fact tables, and an auxiliary segmentation table so that the analytical workflow in Sections 1 and 3 can be supported by a consistent data model.

        \subsubsection{Construction Logic}
        \begin{itemize}
            \item The raw \textit{products.json} and \textit{products.csv} files provide the source product attributes used to construct the product, category, and shop dimensions.
            \item Consecutive daily product snapshots are aligned by product ID and date in order to form the \texttt{fact\_product\_snapshot} table, which stores the time-varying sales and pricing fields used throughout the dashboard.
            \item The review crawler output is reshaped into \texttt{fact\_review}, where each row corresponds to one customer review linked back to the relevant product and date.
            \item Date attributes are standardized into a dedicated \texttt{dim\_date} table so that both sales and review facts can share the same analytical calendar.
            \item Product-level clustering results are exported separately into \texttt{product\_segments}, then linked back to the conformed product dimension for the machine learning page.
        \end{itemize}

        \subsubsection{Relational Model Used in Power BI}
        Based on the implemented Power BI model shown in the relationship view, the final analytical dataset uses the following main tables:

        \begin{itemize}
            \item \texttt{dim\_category}: category reference table keyed by \texttt{category\_id}
            \item \texttt{dim\_shop}: shop reference table keyed by \texttt{shop\_id}
            \item \texttt{dim\_date}: shared calendar table keyed by \texttt{date\_key}
            \item \texttt{dim\_product}: conformed product dimension keyed by \texttt{product\_id}
            \item \texttt{fact\_product\_snapshot}: sales snapshot fact table at the product-date level
            \item \texttt{fact\_review}: review fact table at the individual-review level
            \item \texttt{product\_segments}: product-level clustering output table
        \end{itemize}

        The active relationships visible in the model can be summarized as follows:

        \begin{itemize}
            \item \texttt{dim\_category(category\_id)} $1 \rightarrow *$ \texttt{dim\_product(category\_id)}
            \item \texttt{dim\_shop(shop\_id)} $1 \rightarrow *$ \texttt{dim\_product(shop\_id)}
            \item \texttt{dim\_product(product\_id)} $1 \rightarrow *$ \texttt{fact\_product\_snapshot(product\_id)}
            \item \texttt{dim\_product(product\_id)} $1 \rightarrow *$ \texttt{fact\_review(product\_id)}
            \item \texttt{dim\_date(date\_key)} $1 \rightarrow *$ \texttt{fact\_product\_snapshot(date\_key)}
            \item \texttt{dim\_date(date\_key)} $1 \rightarrow *$ \texttt{fact\_review(date\_key)}
            \item \texttt{dim\_product(product\_id)} $1 \rightarrow 1$ \texttt{product\_segments(product\_id)}
        \end{itemize}

        This structure gives the project a star-schema style core for descriptive analysis, while also attaching the clustering output as a product-level extension. As a result, category analysis, finance structure, product influence analysis, review signals, and clustering insights can all be supported from the same integrated analytical model.
    ...
    This section outlines the data cleaning and feature engineering procedures. Please note that the subsequent Exploratory Data Analysis (EDA) and its visualizations are documented in a separate supplementary file (\textit{\textbf{eda.ipynb}}).
        ...
        This dataset captures the overarching catalog details and aggregate performance metrics for each item in the flagship store. In the raw product metadata snapshot, each row represents one unique product collected during a crawling run.
        \begin{itemize}
            ...
            \item \textbf{ton\_kho (Inventory Status)}: A categorical indicator specifying whether the item is currently In-Stock or Out-of-Stock.
        ...
        \begin{itemize}
            ...
            \item \textbf{Geographical Standardization}: Merged naming inconsistencies in the warehouse location (dia\_diem\_kho), specifically mapping "Ho Chi Minh" to the standardized "Thanh pho Ho Chi Minh".
        ...
        The extracted review texts and numerical ratings were largely retained in their original state, with only lightweight structural normalization during extraction such as merging multipart review content and trimming text fields where necessary.
